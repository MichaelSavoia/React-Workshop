import { db, auth, mode } from 'app/db.js';
...
export function calculateExpectedMinutes(user) {
  const days = differenceInDays(new Date(), parseISO(user.started));
...
export function calculateWeeks(posts, startDate, numWeeks) {
  ...
  const startDay = startOfWeek(
    subDays(parseISO(startDate), (numWeeks - 1) * 7)
  );
  let weekCursor = -1;
...
function limitCalls(fn, limit = 20) {
  ...
  return (...args) => {
    ...
    if (calls > limit) {
      throw new Error(
        `EASY THERE: You've called "${fn.name}" too many times too quickly, did you forget the second argument to useEffect? Also, this is a message from Michael, not React.`
      );
    } else {
